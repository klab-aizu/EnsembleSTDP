# Execution example: python3 averaging_2_models_alpha_gamma.py --method mse --alpha 0.0030 --gamma 0.0060
import argparse
import os
...
parser.add_argument("--alpha", dest="alpha", default = "0")
parser.add_argument("--gamma", dest="gamma", default = "0")
...
for i, j, val in pairs:
    if method == "mse" or method == "manhattan":
        if val < alpha:
            removes.add(j)
        elif val < gamma:
            removes.add(j)
...
print(f"Method: {method}, alpha: {alpha}, gamma: {gamma}, n_compression: {n_compression}, Accuracy: {acc}")
